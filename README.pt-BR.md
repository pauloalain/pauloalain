🌐 This document is also available in [English](./README.md).

# Paulo Alain de Moura Quérette

### 💻 Estudante de Engenharia de Software | 📊 Entusiasta de Análise de Dados | 🧠 Pesquisa & Tecnologia

## 🚀 Sobre Mim

Sou estudante de Engenharia de Software, apaixonado por tecnologia, análise de dados, desenvolvimento de software e pesquisa científica.

Minha trajetória profissional une **Psicologia**, **Pesquisa Científica** e **Tecnologia**, proporcionando-me uma perspectiva multidisciplinar focada na resolução de problemas, pensamento analítico e soluções centradas no ser humano.

Atualmente cursando:

- 🎓 Bacharelado em Engenharia de Software
- 🎓 Tecnólogo em Sistemas de Informação (CEDERJ/UFF)
- 📈 Certificado Profissional de Análise de Dados do Google

Estou em busca de oportunidades em:

- Desenvolvimento de Software
- Análise de Dados
- Projetos de Tecnologia
- Pesquisa & Inovação
- Automação de Processos

---

## 🛠️ Tecnologias & Competências

### Linguagens & Tecnologias
- Python
- JavaScript
- HTML5
- CSS3
- SQL (Aprendendo)
- Git & GitHub
- Excel
- SPSS

### Conceitos
- Programação Orientada a Objetos (POO)
- Análise de Dados
- Análise Estatística
- Fundamentos de Desenvolvimento de Software
- Sistemas Operacionais
- Metodologia de Pesquisa Científica

### Soft Skills
- Pensamento Crítico
- Raciocínio Lógico
- Criatividade
- Resolução de Problemas
- Comunicação
- Visão Interdisciplinar

---

## 📚 Formação Acadêmica

### 🎓 Universidade de Vassouras
**Bacharelado em Engenharia de Software**  
2025 — 2028

### 🎓 CEDERJ / UFF
**Tecnologia em Sistemas de Informação**  
2026 — 2028

### 🎓 Certificado Profissional de Análise de Dados do Google
Coursera — 2026

### 🎓 Universidade Federal do Rio de Janeiro (UFRJ)
**Graduação em Psicologia**  
2012 — 2017

---

## 🔬 Experiência em Pesquisa

Atuei no projeto de pesquisa em tecnologia assistiva:

### “Navegando no Universo Virtual com o Radar Tátil”

O projeto focou na criação de um dispositivo eletrônico capaz de gerar percepção espacial virtual por meio de vibrações ao redor da cabeça para pessoas com deficiência visual.

### Contribuições:
- Coleta e análise de dados
- Tratamento estatístico utilizando:
  - Qui-Quadrado de Pearson
  - ANOVA de Medidas Repetidas
  - Análise de Concordância Interobservadores
- Colaboração em artigo científico
- Pesquisa com participantes deficientes visuais no Instituto Benjamin Constant

Colaboração internacional envolvendo pesquisadores do:
- 🇧🇷 Brasil
- 🇫🇷 França
- 🇯🇵 Japão

---

## 📊 Áreas de Interesse

- Ciência de Dados
- Inteligência Artificial
- Engenharia de Software
- Interação Humano-Computador
- Tecnologia Assistiva
- Computação Científica
- Pesquisa em Tecnologia Comportamental

---

## 🌎 Idiomas

- Português 🇧🇷 (Nativo)
- Inglês 🇺🇸
- Espanhol 🇪🇸
- Francês 🇫🇷

---

## 📫 Contato

- 📧 pauloalain@gmail.com
- 💼 LinkedIn: https://linkedin.com/in/pauloalain
- 📍 Maricá, Rio de Janeiro - Brasil

---

<div align="center">

### “A tecnologia combinada com a compreensão humana cria as melhores soluções.”

</div>
